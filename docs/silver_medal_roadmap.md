...
> コンペ締切: 2026-08-05  
> **自分の現在スコア: 28.341**（2026-06-17 初回提出）
...
| 週 | 期間 | マイルストーン | 目標LBスコア | 実績 |
|----|------|--------------|------------|------|
| Week 1 | 6/17 - 6/22 | ベースライン提出・EDA完了 | < 12.0 | **28.341 ✅** |
...
1. ✅ **初回提出完了（2026-06-17）: スコア 28.341**
   - 手法: テスト well の最後の既知 TVT_input から傾き 0.01 ft/ft でクリップした線形外挿
   - 評価は hidden dataset で行われる（公開 3 well とは別の well セット）
   - Kaggle kernel 経由提出（`kaggle competitions submit -k` + `-f submission.csv -v N`）
2. **DWT-based NB（603票）をフォーク**して Kaggle 上で実行・スコア確認（次のアクション）
